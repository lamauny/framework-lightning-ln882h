...
#include "store_kv_api.h"
#include "store_kv_config.h"
...
#if ( FLASH_LOG_ENABLE )
#define flash_log(fmt,arg...)   printf(fmt,##arg)
#else
#define flash_log(fmt,arg...)
#endif

static int flash_set_with_kv(const char *module_str, uint32_t addr, uint8_t *pbuf, int32_t len)
{
    char *key = NULL;
    char *value = NULL;
    int i = 0;
    ...
    uint32_t index = addr - KV_PARTITION;
    uint8_t index_bcd[5];
    ...
    key = malloc( KV_MODULE_KEY_LENGTH );
    if( key == NULL )
    {
        flash_log("malloc failed \r\n");
        goto exit;
    }
    value = malloc( len * 2 + 1 );
    if( key == NULL )
    {
        flash_log("malloc failed \r\n");
        goto exit;
...
static int flash_get_with_kv(const char *module_str, uint32_t addr, uint8_t *pbuf, int32_t len)
{
    char *key = NULL;
    char *value = NULL;
    int i = 0;
...
int at_wlan_flash_write_kv(uint32_t addr, uint8_t *pbuf, int32_t len)
{
    return flash_set_with_kv(AT_WLAN_STR_HEADER, addr, pbuf, len);
}

int at_wlan_flash_read_kv(uint32_t addr, uint8_t *pbuf, int32_t len)
{
    return flash_get_with_kv(AT_WLAN_STR_HEADER, addr, pbuf, len);
}


int s907x_write_kv(char *key, uint8_t *pbuf, int32_t len)
{
	int err;
...
int s907x_read_kv(char *key, uint8_t *pbuf, int32_t len)
{	
	int err;
